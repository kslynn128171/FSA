...
opt.mass=0;
opt.tol=0.003;
ppm=[0.0013,0.0006,0.0005,0.0029,0.0013,0.0011,0.0011];
opt.MaxPeak=30;
outputname='Supplementary_File1.xlsx';
datasets={'HMDB_QTOF_spectra';
    ...
    'PNNLLIPID_spectra'};
sheetnames={'HMDB_QTOF','HMDB_ITFT','HMDB_QFT','NPL_MAXIS','NPL_QTOF','NPL_OT','LIPID_OT'};
noelem=[0,0,0,6,6,6,5];
...
for d=1:dlen
    ...
    name=extractfield(spectra,'monisotopic_molecular_weight');
    if noelem(d)>0
    ...
    disp(['Dataset ',num2str(d),' used ',num2str(usedtime),' secs.'])
    ...
    %rank1(5,(2*(d-1)+1):(2*d))={num2str(applicablespect),num2str(applicablecomp)};
