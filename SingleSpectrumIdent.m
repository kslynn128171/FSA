function msg=SingleSpectrumIdent(spectidx, elemmtx, formulist, mass, spectrum, opt)
% SingleSpectrumIdent Rank the precursor formula candidates using formula
% subset analysis.
% input parameters: 
% spectidx: spectrum index
% elemmtxa: the matrix that contains C, H, O, N, S, and P element numbers for compounds in PubChem
% formulist: the cell array that contains compound formulas in PubChem
...
for j=1:nop
    ...
    if j == 1 % check for the precursor
        ...
        tf=strcmp(formulist.Value(idx),spectrum.chemical_formula);
...
for j=1:noc % for each precursor formula candidate
    vecp=elemmtx.Value(idrec{1}(j),:); % element composition in the precursor formula
    cnum=ones(nop,1);
    uidrec=cell(nop,1);
    for k=2:nop % make sure that all the fragment components are a subset of those of the precursor
        idx=idrec{k};
        matf=elemmtx.Value(idx,:); % fragment matrix
    ...
    tempscore=sum(all(elemmtx.Value(uidrec(k),:) >= elemmtx.Value(uidrec(l),:),2));
