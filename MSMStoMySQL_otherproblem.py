...
cursor = conn.cursor()
print("Connect Mysql")
#tables = '''create table msmsdata20181215(collection_date text ,collision_energy_level text ,
                ...
                for REF_L2 in REF_L1:
                    for REF_L3 in REF_L2.findall(label2[j]) :
                        if len(REF_L3)==0 and REF_L3.text==None:
                            ...
                            text=REF_L3.text                
                            subdata.append(text)
    tempdata = [subdata[0],subdata[1]]
    tempdata2 = [subdata[2],subdata[3]]
    tempdata3 = [subdata[4],subdata[5]]
    tempdata4 = [subdata[6],subdata[7]]
    tempdata5 = [subdata[8],subdata[9]]
    tempdata6 = [subdata[10],subdata[11]]
    tempdata7 = [subdata[12],subdata[13]]
    combinedata2 = ";".join(tempdata)
    combinedata3 = ";".join(tempdata2)
    combinedata4 = ";".join(tempdata3)
    combinedata5 = ";".join(tempdata4)
    combinedata6 = ";".join(tempdata5)
    combinedata7 = ";".join(tempdata6)
    combinedata8 = ";".join(tempdata7)
    combine_subdata2.append(combinedata2)
    combine_subdata2.append(combinedata3)
    combine_subdata2.append(combinedata4)
    combine_subdata2.append(combinedata5)
    combine_subdata2.append(combinedata6)
    combine_subdata2.append(combinedata7)
    combine_subdata2.append(combinedata8)
    if len(root.findall('ms-ms-peaks')) == 0 and root.text.strip()=='' :
        text="no exist"
        combine_subdata2.append(text)
    else:
        for peak_L1 in root.findall('ms-ms-peaks'):
            ...
            combine_subdata2.append('\"'+combine_peakdata2+'\"')
    data.insert(6,'')
    data.insert(9,'')
    data.insert(15,'\"'+str(combine_subdata2[0])+'\"')
    if len(data)+len(combine_subdata2) != 27:
        print(filename)
    else:
        for n in range(1,8,1):
            data.insert(n+18,combine_subdata2[n])
