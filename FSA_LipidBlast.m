...
close all
fclose all;
output_filename='FSA_result_LipidBlast_CHONSP_0004.xlsx'; % file name for the identification result
% set parameters
opt.TINY=0; % ratio threshold for "tiny" peaks
opt.PPM=0.0004;% mass match tolerance for QTOF, ITFT, and QFT
...
varTypes = {'double','string','string','string','double','string','double','double','double','double',...
        ...
        'double','string'};
% the test datasets
Tb = readtable('LipidBlast_all_CHONSP.xlsx','FileType','spreadsheet','TextType','string','VariableNamingRule','modify');
spectnum=size(Tb,1);
Tb.instrument_type=repmat({'none'},spectnum,1);
...
%     end
    ...
    waitbar(ratio, h, ['In LipidBlast: ',num2str(ratio*100,'%6.2f'),'% (',num2str(i),'/',num2str(spectnum),') finished']);
end
totaltime=cputime-starttime
% write FSA results
writetable(spect,output_filename,'Sheet','LipidBlastResult');
% prepare summary of FSA results
vName={'SpectrumNum(Percentage)','CompoundNum(Percentage)'};
vType={'string','string'};
rName={'Total','Problematic','No_Answer_within_Candidates','No_MDR','Total_Identifiable','Solo_Candidate','Top-1_Rank','Top-3_Rank','Top-5_Rank'};
...
%save FSA_LipidBlast_result_tables_001_new.mat spect rank1
delete(h);