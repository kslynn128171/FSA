...
for j=1:nop
    ...
    if j == 1 && mz(j) >= 1000 % precursor larger than 1000 has only two decimal digits
        delta=0.005;
    elseif opt.MatchTolerance < 1 % use Da as unit of the tolerance
...
if nof == 1
    % No MDR, perform regular mass match instead
